...
export abstract class YouTubeMessage {
  ...
  checkBufferIsAd (filed): boolean {
    ...
    const data = filed.data as Uint8Array
    const needle = [112, 97, 103, 101, 97, 100]
    const skip = new Int32Array(256).fill(needle.length + 1)

    for (let i = 0; i < needle.length; i++) {
      skip[needle[i]] = needle.length - i
    }
    ...
    for (let i = 0; i <= data.length - needle.length;) {
      ...
      i += skip[data[i + needle.length]] || needle.length + 1
    }
